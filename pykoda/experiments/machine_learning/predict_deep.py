...
def get_deep_model(depth=2, width=16, n_features=2, l2_reg=1e-6):
    if l2_reg > 0:
        reg = l2(l2_reg)
    else:
        reg = None
...
def main():
    ...
    train_X, train_y = get_features(COMPANY_NAME, DATE_TRAIN, START_HOUR, END_HOUR, FEATURES, n_select=SAMPLES)
    test_X, test_y = get_features(COMPANY_NAME, DATE_TEST, START_HOUR, END_HOUR, FEATURES, n_select=SAMPLES)
    ...
    model = get_deep_model(width=WIDTH, depth=1, n_features=train_X[0].shape[1])
    ...
    predicted = np.concatenate([model(x[:, np.newaxis], training=False).numpy().squeeze() for x in test_X])
    truth = np.concatenate(test_y)

    predicted_train = np.concatenate([model(x[:, np.newaxis], training=False).numpy().squeeze() for x in train_X])
    truth_train = np.concatenate(train_y)
