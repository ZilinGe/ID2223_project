...
def bz2_to_tar(bz2_file):
    ...
    decompressed_file = decompressed_file.replace("\\", "/")
    tar_file = os.path.join(folder, f"{base_name}.tar")
    tar_file = tar_file.replace("\\", "/")

    try:
        # 调用系统工具解压 .bz2 文件
        subprocess.run(["7z", "e", "-o" + decompressed_file, bz2_file], check=True)  # -k 保留原始 .bz2 文件
        #print(f"Decompressed using system tool: {bz2_file} to {decompressed_file}")
